...
add enrichment to route

enrichments?: {
    inbound?: EnrichmentStep[];
    canonical?: EnrichmentStep[];
    outbound?: EnrichmentStep[];
  };
...
}
{
  enrichments: {
    ...
    outbound: [
      {
        id: 'generate-accession',
        type: 'openelis-accession'
      }
    ]
  }
}
...
Runs BEFORE canonical transformation.

Used for:

raw payload normalization
extracting custom fields
preprocessing HL7 Z-segments
facility identification
...
raw HL7
   ↓
extract ZRT target AE
extract facility code
normalize malformed payload
Canonical

Runs AFTER canonical transformation.
...
Runs BEFORE serialization/dispatch.

Used for:

OpenELIS accession formatting
HL7 facility code population
PACS AET lookup
DICOM formulation
destination-specific computed values

Example:

canonical
   ↓
resolve PACS destination AE
resolve HL7 location code
generate accession number

create codingConceptClient as enrichment provider - TerminologyEnrichmentProvider

provider types include

TerminologyEnrichmentProvider
FacilityEnrichmentProvider
PractitionerEnrichmentProvider
OpenElisAccessionProvider
DicomWorklistProvider

test above with making sure Fhir order is sent in this format
{
  "resourceType": "ServiceRequest",
  "id": "custom-rad-1",
  "identifier": [
    {
      "system": "urn:healthstack:order",
      "value": "custom-rad-1"
    }
  ],
  "status": "active",
  ...
  "subject": {
    "reference": "Patient/patient-rad-1"
  },
  "authoredOn": "2026-04-25T09:00:00Z",
  "requester": {
...
After the above, use @types/fhir
 to create and validate Fhir resources
